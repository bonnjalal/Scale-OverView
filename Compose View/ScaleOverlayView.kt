...
import com.bonnjalal.pdfcropper.tools.DeviceDimensionsHelper
import com.bonnjalal.pdfcropper.tools.DeviceDimensionsHelper.convertDpToPixel
...
fun scaleOverView(
    ...
    Canvas(modifier = drawModifier) {
        ...
        when (motionEvent) {
            ACTION_DOWN -> {
                ...
                if (points[0] == null) {
                    ...
                    groupId = getCurrentTouchIndex(currentPosition.x, currentPosition.y, mCropRect, mTPT, enableDrag )
                    //invalidate()
                }
            }
            ACTION_MOVE -> {
        ...
        if (cornerBitmap != null){
            ...
            var i = 0
            while (i < 8) {
                drawImage(image = cornerBitmap!!,
                    topLeft = Offset(mCropGridCorners[i] - (cornerBitmap.width/2),
...
fun getCurrentTouchIndex(touchX: Float, touchY: Float,
                         mCropRect: RectF,
                         mTouchPointThreshold: Float,
                         mEnableDrag: Boolean): Int {
